...
namespace P072G3A_FuncTest
{
    ...
    public class ApsController : Automatic, ISwitchController, IMotionController, INeedInitialization, IDisposable
    {
        ...
        public void SetSoftConfig(int axisNo, double softPositiveLimit, double softNegativeLimit)
        {
            APS168.APS_set_axis_param_f(axisNo, (int)APS_Define.PRA_SPEL_POS, softPositiveLimit);
            APS168.APS_set_axis_param_f(axisNo, (int)APS_Define.PRA_SMEL_POS, softNegativeLimit);
        ...
        public void SetAxisAccDecVel(int axisNo, double accVel, double decVel)
        {
            //设置速度
            SetAxisParam(axisNo, (int)APS_Define.PRA_ACC, accVel);
            SetAxisParam(axisNo, (int)APS_Define.PRA_DEC, decVel);
        }
        ...
        /// <returns></returns>
        public void MoveRelPulse(int axisNo, int pulseNum, int maxVel)
        {
            //启动运动
            APS168.APS_relative_move(axisNo, pulseNum, maxVel);
        }
        ...
        /// <param name="pulseNum"></param>
        /// <param name="maxVel"></param>
        public void MoveAbsPulse(int axisNo, int pulseNum, int maxVel)
        {
            //启动运动
            APS168.APS_absolute_move(axisNo, pulseNum, maxVel);
        }
        ...
        /// <param name="maxVel"></param>
        /// <returns></returns>
        public void MoveContinu(int axisNo, MoveDirection moveDirection, int maxVel)
        {
            //启动运动
            APS168.APS_velocity_move(axisNo, maxVel * (int)moveDirection);
        }
        ...
        /// </summary>
        /// <param name="axisNo"></param>
        /// <param name="isOn">1=ON,0=OFF</param>
        public void MoveJog(int axisNo,int isOn)
        {
            APS168.APS_jog_start(axisNo, isOn);
        }
        ...
        {
            APS168.APS_set_servo_on(axisNo, 1);
        }

        /// <summary>
        ///     轴掉电
        /// </summary>
        /// <param name="axsiNo"></param>
        public void ServoOff(int axisNo)
        {
            APS168.APS_set_servo_on(axisNo, 0);
        }  
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsMotionless(int axisNo)
        {
            bool isStop = true;
            int status = APS168.APS_motion_status(axisNo);
        ...
        {
            return (GetMotionStatus(axisNo) & 2) == 16;
        }
        ...
        /// </summary>
        /// <param name="axisNo"></param>
        private StopReasons GetStopReason(int axisNo)
        {
            var code = 0;
        ...
        private void loadcif()
        {
            ...
            {
                var innerMsg = "";
                innerMsg = GetErrorCodeDesc((APS_Define)1);
                if (innerMsg != "No Error")
                    MessageBox.Show(string.Format("凌华控制卡功能错误:{0}", innerMsg));
                //ThrowIfResultError(1);
                return;
            }

            //判断配置文件是否存在
            if (File.Exists(AppConfig.AxisParamPath))
            {
                LoadParamFromFile(AppConfig.AxisParamPath);
            }
            else
            {
                LoadParamFromFile("C:\\Users\\Administrator\\Desktop\\param204.xml");
            }
        }

        private bool LoadParamFromFile(string xmlfilename)
        {
        ...
        public int RelativeMove(int axisNo, double position, double maxVel)
        {
            SetAxisParam(axisNo, 36, maxVel);
            ...
            //innerMsg = GetErrorCodeDesc((APS_Define)ret);
            //if (innerMsg != "No Error")
            //    MessageBox.Show(string.Format("凌华控制卡相对运动功能错误:{0}", innerMsg));
            //ThrowIfResultError(ret);
            return ret;
        }
        //绝对值移动
        public int AbsoluteMove(int axisNo, double position, double maxVel)
        {
            SetAxisParam(axisNo, 36, maxVel);
        ...
        {
            //return ((GetMotionIoStatus(axisNo) >> (int) APS_Define.MIO_SVON) & 1) > 0;
            return true;
        }
