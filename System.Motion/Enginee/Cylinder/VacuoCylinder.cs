...
{
    public class VacuoCylinder : Cylinder
    {
        #region "字段"
        ...
        public VacuoCylinder(Sensor InSensor, Signal OutVacuo)
        {
            _InSensor = InSensor;
            _OutVacuo = OutVacuo;
        ...
        public VacuoCylinder(CylinderCondition cylinderCondition, VacuoDelay vacuoDelay, Sensor InSensor, Signal OutVacuo)
            :this( InSensor,  OutVacuo)
        {
            ...
            get
            {
                lock (this)
                {
                    ...
                    || Condition.NoOriginShield))
                        if (Delay.BrokenTime <= _watchInhale[0].ElapsedMilliseconds) return true;
                        else return false;
                    else
                    {
                        _watchInhale[0].Restart();
        ...
        public override bool OutMoveStatus
        {
            get
            {
                lock (this)
                {
                    ...
                    if (_OutVacuo.Value && ((_InSensor.Value && !Condition.NoOriginShield)
                    || Condition.NoOriginShield))
                        if (Delay.InhaleTime <= _watchInhale[1].ElapsedMilliseconds) return true;
                        else return false;
                    else
                    {
                        _watchInhale[1].Restart();
