...
namespace Motion.Enginee
{
    public class StationInitialize : IStationInitialize
    {
        ...
        private InilitiazeStatus m_Status;
        #endregion
        ...
        /// 初始化状态
        /// </summary>
        public InilitiazeStatus Status { get { return m_Status; } }
        /// <summary>
        /// 流程步骤
        /// </summary>
        ...
        public void Run()
        {
            ...
            //判断是否运行
            if ((m_Start || m_Running) && _condition && !_isAlarm && m_RunSignal) m_Running = true;
            else m_Running = false;
            //判断是否初始化完成
            if (InitializeDone || _isAlarm) m_RunSignal = false;
            //初始化状态
            if (_isAlarm) m_Status = InilitiazeStatus.初始化故障;
            if (!_isAlarm && !m_RunSignal && !m_Running && !InitializeDone) m_Status = InilitiazeStatus.初始化未准备好;
            if (!_isAlarm && InitializeDone) m_Status = InilitiazeStatus.初始化完成;
            if (!_isAlarm && m_RunSignal && m_Running) m_Status = InilitiazeStatus.初始化运行中;
            if (!_isAlarm && m_RunSignal && !m_Running) m_Status = InilitiazeStatus.初始化暂停中;
        }
    }
}
